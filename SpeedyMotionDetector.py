...
# To run, execute 
# >> python SpeedyMotionDetector.py

# Comment out videos you don't want to see and uncomment those you do in the main function 
# Press any key to move onto the next frame
# Set show_all to True if you want to see all moving objects and their score
# Set show_mask to True if you want to see the motion mask 

class SpeedyDetector:
    ...
    def __init__(self, filename, fps, aspect_ratio):
        ...
        self.color_video = []
        self.read_video(filename, captured_fps=fps)
        self.id_counter = 0
        self.obj_hist = {}
        self.motion_objs_stamped = []
        self.fast_objs = []
        self.show_all=self.show_mask=False
        self.show_fast=True

    def read_video(self, filename, captured_fps=30, frame_limit=10000):
        ...
        self.output = self.color_video.copy()

    def detect_falling(self, channel=2, threshold=0.15, open_size=3, close_size=7, min_area=0.0, wH=0.33, wS=0.33, wV=0.33, min_speed=200.0, similarity=0.03, post_morph=False):
    ...
    def show_motion(self, frame_idx):
        ...
        if self.show_fast:
            for obj in self.fast_objs:
                ...
                cv.putText(self.output[frame_idx], 'Falling!', (obj[2][0], obj[2][1]), cv.FONT_HERSHEY_SIMPLEX, 0.5, (63,255,255), 1, 2)
                cv.rectangle(self.output[frame_idx], (obj[2][0], obj[2][1]), (obj[2][0]+obj[2][2]-1, obj[2][1]+obj[2][3]-1), (63, 255, 255), 1) # green
        if self.show_mask:
    ...
    def output_video(self):
        out = cv.VideoWriter(self.filename[:-4]+"_out.avi", cv.VideoWriter_fourcc(*'DIVX'), 15, (self.resolution[1], self.resolution[0]))
        for i in range(len(self.output)):
            out.write(cv.cvtColor(self.output[i].astype(np.uint8), cv.COLOR_HSV2BGR))
        out.release()
        print("Output video saved as:", self.filename[:-4]+"_out.avi")

if __name__ == "__main__":
    # falling_objs = SpeedyDetector("falling_objects.mov", 30, (16, 9))
    # falling_objs.show_all = False 
    # falling_objs.show_mask = False 
    # falling_objs.detect_falling(channel=2, threshold=0.08, open_size=5, close_size=9, min_area=0.0005, wH=0.15, wS=0.25, wV=0.6, min_speed=300.0, similarity=0.05, post_morph=True)

    falling_simult = SpeedyDetector("falling_simult.mov", 30, (16, 9))
    falling_simult.show_all = False
    falling_simult.show_mask = False 
    ...
    # nonfalling_motion = SpeedyDetector("nonfalling_motion.mov", 30, (16, 9))
    # nonfalling_motion.show_all = False
    # nonfalling_motion.show_mask = False 
